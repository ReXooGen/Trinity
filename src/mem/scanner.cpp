...
namespace trinity::mem
{
    ...
    namespace
    {
                      ...
                      std::vector<uintptr_t>& out, size_t maxResults)
        {
            ...
            for (const uint8_t* p = start; p <= last; ++p)
            {
                ...
                {
                    hit = false;
                    // If we hit an unreadable page, skip to the next page to avoid constant exceptions
                    uintptr_t next_page = ((reinterpret_cast<uintptr_t>(p) + 0x1000) & ~0xFFF);
                    if (next_page > reinterpret_cast<uintptr_t>(p))
                    {
                        p = reinterpret_cast<const uint8_t*>(next_page - 1); // -1 because loop will ++p
                    }
                }
        ...
        uintptr_t ScanSpanIf(uintptr_t begin, uintptr_t end, const ParsedPattern& pat,
                             size_t firstFixed, uint8_t anchor, bool haveAnchor,
                             bool (*visit)(uintptr_t match, void* ctx), void* ctx)
        {
            const size_t patLen = pat.bytes.size();
            if (end < begin || (end - begin) < patLen) return 0;
            ...
            for (const uint8_t* p = start; p <= last; ++p)
            {
                ...
                __try
                {
                    ...
                    if (hit)
                    {
                        ...
                        __try
                        {
                            visitResult = visit(reinterpret_cast<uintptr_t>(p), ctx);
                        }
                        __except (EXCEPTION_EXECUTE_HANDLER)
                        {
                            // False positive caused an access violation inside visit(), ignore.
                            visitResult = false;
                        }
                        if (visitResult)
                ...
                __except (EXCEPTION_EXECUTE_HANDLER)
                {
                    ...
                    if (next_page > reinterpret_cast<uintptr_t>(p))
                    {
                        p = reinterpret_cast<const uint8_t*>(next_page - 1);
                    }
                }
            }
            return 0;
                            ...
                            bool (*visit)(uintptr_t match, void* ctx), void* ctx)
    {
        ...
        for (const auto& [begin, end] : ReadableSpans(mod))
        {
            uintptr_t res = ScanSpanIf(begin, end, pat, firstFixed, anchor, haveAnchor, visit, ctx);
            if (res) return res;
        }
        return 0;
    }
