...
namespace trinity::game
{
    ...
    namespace
    {
        ...
        bool DefForRow(uintptr_t tableGlobal, uint16_t row, uintptr_t* out)
        {
            ...
            uint32_t count = 0;
            uintptr_t defs = 0;
            // In TU 1.10 - 1.16 legacy: +0x50 is defs[]
        ...
        {
            uintptr_t def = 0;
            if (DefForRow(g_itemTableGlobal, typeId, &def) && StringField(def + kOff_ItemDef_Key, out, n))
                return true;
            const char* fallbackKey = ResolveItemKeyForTypeId(typeId);
            if (fallbackKey && fallbackKey[0] != '\0')
            {
                strncpy_s(out, n, fallbackKey, _TRUNCATE);
                return true;
            }
            return false;
        }
        ...
        {
            uintptr_t def = 0;
            if (DefForRow(g_itemTableGlobal, typeId, &def))
            {
                uintptr_t list = 0;
                uint32_t  count = 0;
                if (ReadPtr(def + kOff_ItemDef_Icons + kOff_Vec_Data, &list) &&
                    Read32(def + kOff_ItemDef_Icons + kOff_Vec_Count, &count) &&
                    list >= kMinPointer && count > 0 && count <= 64)
                {
                    uint16_t row = 0;
                    if (Read16(list + kOff_IconData_Path, &row) && IconNameForRow(row, out, n))
                        return true;
                }
            }
            const char* k = ResolveItemKeyForTypeId(typeId);
            if (k && k[0] != '\0')
            {
                snprintf(out, n, "itemicon_%s", k);
                return true;
            }
            return false;
        }
        ...
        {
            uintptr_t def = 0;
            if (DefForRow(g_invTableGlobal, type, &def) && LocString(def + kOff_InvDef_Name, out, n))
                return true;
            const char* k = nullptr;
            switch (type)
            {
            case 0: k = "Camp"; break;
            case 1: k = "Inventory"; break;
            case 2: k = "Pearl User"; break;
            case 3: k = "Pearl Character"; break;
            case 4: k = "Quest Items"; break;
            case 5: k = "Pet & Vehicle"; break;
            case 6: k = "Wagon"; break;
            case 7: k = "Camp Warehouse"; break;
            case 8: k = "Storage"; break;
            case 9: k = "Bank"; break;
            case 10: k = "Camp Straw"; break;
            case 11: k = "Bird Feeder"; break;
            case 12: k = "Recovery"; break;
            case 13: k = "Wardrobe"; break;
            case 14: k = "Refrigerator"; break;
            case 15: k = "Housing Symbol"; break;
            case 16: k = "Gathering"; break;
            case 17: k = "Materials"; break;
            case 18: k = "Kuku Pot"; break;
            case 19: k = "Invisible"; break;
            default: return false;
            }
            strncpy_s(out, n, k, _TRUNCATE);
            return true;
        }
        ...
        {
            uintptr_t def = 0;
            if (DefForRow(g_invTableGlobal, type, &def) && StringField(def + kOff_InvDef_Key, out, n))
                return true;
            const char* k = nullptr;
            switch (type)
            {
            case 0: k = "Money"; break;
            case 1: k = "Character"; break;
            case 2: k = "PearlUser"; break;
            case 3: k = "PearlCharacter"; break;
            case 4: k = "Quest"; break;
            case 5: k = "PetAndVehicle"; break;
            case 6: k = "Wagon"; break;
            case 7: k = "CampWareHouse"; break;
            case 8: k = "WareHouse"; break;
            case 9: k = "Bank"; break;
            case 10: k = "CampStraw"; break;
            case 11: k = "BirdFeed"; break;
            case 12: k = "Recovery"; break;
            case 13: k = "Housing_Dresser"; break;
            case 14: k = "Housing_Refrigerator"; break;
            case 15: k = "Housing_Symbol"; break;
            case 16: k = "Housing_Collecting"; break;
            case 17: k = "Housing_GatheredMaterials"; break;
            case 18: k = "Kuku"; break;
            case 19: k = "InvisibleInventory"; break;
            default: return false;
            }
            strncpy_s(out, n, k, _TRUNCATE);
            return true;
        ...
        int64_t __fastcall hkGetItemQty(void* container, uint16_t typeId, void* keyPtr)
        {
            ...
            if (g_campSpoofAddedValue > 0)
            {
                ...
                uint16_t tCampTimber = Inventory::FindTypeIdByKey("Money_Camp_Wood");
        ...
        void HexDumpLog(const char* prefix, uintptr_t start, size_t length)
        {
            for (size_t i = 0; i < length; i += 16)
            {
                ...
                {
                    uint8_t b = 0;
                    if (mem::Read8(start + i + j, &b))
                        sprintf_s(hex + j * 3, 4, "%02X ", b);
                    else
                        sprintf_s(hex + j * 3, 4, "?? ");
                }
                LOG_OK("%s %p: %s", prefix, reinterpret_cast<void*>(start + i), hex);
            }
        }
        ...
        bool IsTableRef(uintptr_t match, void* ctx)
        {
            ...
            LOG_OK("inventory: DEBUG FOUND LEA FOR '%s' at %p", h->name, reinterpret_cast<void*>(match));
            HexDumpLog("inventory: dump", match - 0x60, 0x80);

            const uintptr_t fn = FindItemPrologueAbove(match);
            if (fn)
            {
                uint8_t op[3] = {0};
                if (!mem::Read8(fn + kOff_ItemResolver_MovGlobal, &op[0]) ||
                    !mem::Read8(fn + kOff_ItemResolver_MovGlobal + 1, &op[1]) ||
                    !mem::Read8(fn + kOff_ItemResolver_MovGlobal + 2, &op[2]))
                {
                    return false;
                }
    ...
    static void RefreshImplCore(bool force)
    {
        ...
        for (uint32_t b = 0; b < bcount; ++b)
        {
            ...
            for (uint16_t i = 0; i < scount; ++i)
            {
                ...
                char fallbackGroup[48] = "";
                char fallbackTab[32] = "";
                char fallbackIcon[96] = "";
                GetItemCategoryInfo(it.key, fallbackGroup, sizeof(fallbackGroup), fallbackTab, sizeof(fallbackTab), fallbackIcon, sizeof(fallbackIcon));

                Group* g = nullptr;
                for (auto& cand : store.groups)
                {
                    if (it.cat.row != kNoCategory.row)
                    {
                        if (cand.cat.row == it.cat.row) { g = &cand; break; }
                    }
                    else
                    {
                        if (strcmp(cand.label, fallbackGroup) == 0) { g = &cand; break; }
                    }
                }
                if (!g)
                {
                    Group ng{};
                    ng.cat = it.cat;
                    if (!GroupName(it.cat.row, ng.label, sizeof(ng.label)) || strcmp(ng.label, "Uncategorised") == 0)
                        strncpy_s(ng.label, sizeof(ng.label), fallbackGroup, _TRUNCATE);
                    if (it.cat.tabRow == 0xFFFF ||
                        !GroupName(it.cat.tabRow, ng.tab, sizeof(ng.tab)))
                        strncpy_s(ng.tab, sizeof(ng.tab), fallbackTab, _TRUNCATE);
                    if (!IconForGroup(it.cat.row, ng.icon, sizeof(ng.icon)))
                        strncpy_s(ng.icon, sizeof(ng.icon), fallbackIcon, _TRUNCATE);
    ...
    namespace
    {
        ...
        bool CommitAdd(uint16_t typeId, int64_t qty)
        {
            ...
            uintptr_t def = 0;
            const bool haveDef = DefForRow(g_itemTableGlobal, typeId, &def) || (ResolveItemKeyForTypeId(typeId) != nullptr);
            const uintptr_t clientH = CurrentHolder();
    ...
    namespace
    {
        ...
        void BuildCatalog()
        {
            if (g_catalogBuilt && !g_catalog.empty()) return;
            EnsureTablesResolved();

            uintptr_t table = 0;
            uint32_t count = 0;
            bool liveReady = (g_itemTableGlobal != 0 &&
                              ReadPtr(g_itemTableGlobal, &table) && table >= kMinPointer &&
                              Read32(table + kOff_ItemTable_Count, &count) && count > 0 && count <= 65536);
            ...
            if (liveReady)
            {
                ...
                for (uint32_t row = 0; row < count; ++row)
                {
                    ...
                    if (!g)
                    {
                        ...
                        if (!IconForGroup(it.cat.row, ng.icon, sizeof(ng.icon)))
                        {
                            char fGrp[48]{0}, fTab[32]{0};
                            GetItemCategoryInfo(it.key, fGrp, sizeof(fGrp), fTab, sizeof(fTab), ng.icon, sizeof(ng.icon));
                        }
                ...
                if (!g_catalog.empty())
                {
                    ...
                    g_catalogBuilt = true;
                    LOG_OK("inventory: live catalog built - named=%u groups=%u.",
                           static_cast<unsigned>(named), static_cast<unsigned>(g_catalog.size()));
                    return;
                }
            }

            // Static Database Fallback (zero-failure catalog for Legacy 1.14)
            g_catalog.clear();
            uint32_t named = 0;
            for (uint32_t tid = 1; tid < kKnownItemCount; ++tid)
            {
                const char* k = ResolveItemKeyForTypeId(static_cast<uint16_t>(tid));
                if (!k || k[0] == '\0') continue;

                Item it{};
                it.typeId = static_cast<uint16_t>(tid);
                strncpy_s(it.key, sizeof(it.key), k, _TRUNCATE);

                const char* disp = ResolveItemDisplayName(k);
                if (disp && disp[0] != '\0')
                    strncpy_s(it.name, sizeof(it.name), disp, _TRUNCATE);
                else
                    Prettify(k, it.name, sizeof(it.name));

                if (!it.name[0]) continue;
                snprintf(it.icon, sizeof(it.icon), "itemicon_%s", k);
                it.tier = 0;
                it.cat = kNoCategory;
                ...
                char tabName[32] = { 0 };
                char grpIcon[96] = { 0 };
                GetItemCategoryInfo(k, grpLabel, sizeof(grpLabel), tabName, sizeof(tabName), grpIcon, sizeof(grpIcon));

                Group* g = nullptr;
                for (auto& cand : g_catalog)
                {
                    if (strcmp(cand.label, grpLabel) == 0)
                    {
                        g = &cand;
                        break;
                    }
                ...
                if (!g)
                {
                    Group ng{};
                    ng.cat = kNoCategory;
                    strncpy_s(ng.label, sizeof(ng.label), grpLabel, _TRUNCATE);
                    strncpy_s(ng.tab, sizeof(ng.tab), tabName, _TRUNCATE);
                    strncpy_s(ng.icon, sizeof(ng.icon), grpIcon, _TRUNCATE);
                    g_catalog.push_back(std::move(ng));
                    g = &g_catalog.back();
                }
                g->items.push_back(it);
            ...
            if (!g_catalog.empty())
            {
                ...
                g_catalogBuilt = true;
                LOG_OK("inventory: static database catalog built - named=%u groups=%u.",
                       static_cast<unsigned>(named), static_cast<unsigned>(g_catalog.size()));
            }
        }
    ...
    bool Inventory::SetDirectSilver(int64_t silverAmount)
    {
        ...
        // Run background thread scan (from Trinity-1.2.1 / WeMod heap scanner)
