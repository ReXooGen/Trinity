        ...
        if tblDesc:
            ...
            if arr and cnt < 50:
                for k in range(cnt):
                    ...
                    if tid > 0:
                        items.append(f"tag{slotTag}:id{tid}")
                        
    if obj50 in [1, 2, 3, 4, 5] or has19 or tag in [1, 4, 9]:
        print(f"[{i}] owner=0x{ch:X} act=0x{actor:X} tag={tag} obj50={obj50} objType={objType} hp={hp} has19={has19} has22={has22}")
        print(f"     poss=0x{poss:X} items={items}")
